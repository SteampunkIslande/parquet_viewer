# From https://stackoverflow.com/questions/74744879/list-all-python-functions-within-a-vscode-workspace-that-are-uncalled
    ...
    for dir_name, subdir_list, file_list in os.walk(root_dir):
        for file_name in file_list:
            if file_name.endswith(".py"):
                ...
                if "build/lib" not in full_path and "site-packages" not in full_path:
                    filepaths.append(full_path)
    return list(set(filepaths))


@typechecked
def find_function_calls(filepath: str, found_function_calls: List[str]) -> List[str]:
